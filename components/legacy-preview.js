/*
 * Rewrite this part in React eventualy
 * Right now the following code runs on
...
class FloatingPreview {
  ...
  animateShake() {
    const el = this.$instance.find(".preview__card");
  ...
  initialize() {
    const previewMode = this.$reference.attr("data-preview-mode");
